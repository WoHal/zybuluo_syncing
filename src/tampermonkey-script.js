...
// @require      https://unpkg.com/wohal/zybuluo-syncing@main/libs/octokit.js
...
(function() {
    ...
    // 使用节流包装syncToGithub函数，10秒内只执行最后一次
    const throttledSyncToGithub = throttle(syncToGithub, 10000);

    function autoSync() {
        ah.proxy({
            //请求发起前进入
            onRequest: (config, handler) => {
                if (/note\/update/i.test(config.url)) {
    ...
    async function syncToGithub(title, newContent) {
        try {
            ...
            // 创建Octokit实例
            const octokit = new window.Octokit.Octokit({
                auth: apiToken
            });
    ...
    function createUIs() {
        ...
        a.innerText = '下载所有md文件';
        a.onclick = downloadAllMarkdowns;
        li.title = '下载所有md文件';
